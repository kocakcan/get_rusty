...
///
/// You might be wondering whether there is a runtime cost when using generic type parameters. The
/// good news is that using generic types won't make your program run any slower than it would with
/// concrete types.
///
...
/// the concrete types that are used when compiled. In this process, the compiler does the opposite
/// of the steps we used to create the generic function in Listing 10-5: the compiler looks at all
/// the places where generic code is called and generates code for the concrete types the generic
/// code is called with.
///
...
///     let integer = Some(5);
///     let float = Some(5.0);
/// When Rust compiles the code, it performs monomorphization. During that process, the compiler
...
///
/// The monomorphized version of the code looks similar to the following (the compiler uses
/// different names than what we're using here for illustration):
///
///     enum Option_i32 {
///         Some(i32),
///         None,
...
/// Because Rust compiles generic code into code that specifies the type in each instance, we pay
/// no runtime cost for using generics. When the code runs, it performs just as it would if we had
/// duplicated each definiton by hand. The process of monomorphization makes Rust's generics
